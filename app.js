...
app.use(session({
  ...
  resave: false,
  saveUninitialized: false, // 세션이 필요할 때만 저장하도록 설정ㄱ
  store: new fileStore({
  ...
  cookie:{
    ...
    maxAge: 24 * 60 * 60 * 1000 // 쿠기 유효 기간 (1일)
  }
}))
...
async function connectDB() {
  ...
  try {
    ...
    consol.error('Database connection failed: ', error);
    process.exit(1);
  }
}

connectDB().catch(err => {
  consol.error('Failed to connect to the database: ',err);
  process.exit(1);
})
